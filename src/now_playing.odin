...
draw_now_playing_view :: proc(x, y, w, h: f32) {
	...
	if has_lyrics && ui_state.lyrics_animation_progress > 0.001 {
		...
		lyrics_panel_x :=
			...
			content_split +
			20 +
			(w - content_split - 40 - lyrics_panel_w) +
			(1 - ui_state.lyrics_animation_progress) * w * 0.5
		lyrics_panel_y := y + 40
		lyrics_panel_h := h - 80
...
draw_lyrics :: proc(x, y, w, h: f32, track: ^Track, alpha: f32) {
	...
	for lyric, i in track.lyrics {
		...
		if is_current {
			fx.draw_circle(
				x + 15,
				line_y + current_line_height / 2 + 1,
				2.5,
				set_alpha(UI_SECONDARY_COLOR, alpha),
			)
		}
	...
	ui_state.lyrics_scrollbar.target = clamp(
		ui_state.lyrics_scrollbar.target,
		0,
		lyrics_max_scroll,
	)
	ui_state.lyrics_scrollbar.scroll = clamp(
	...
	if lyrics_max_scroll > 0 && ui_state.show_lyrics {
		...
		draw_scrollbar(
			&ui_state.lyrics_scrollbar,
			scrollbar_x,
			scrollbar_y,
			4,
			scrollbar_h,
			lyrics_max_scroll,
			set_alpha(UI_PRIMARY_COLOR, 0.8),
			set_alpha(UI_SECONDARY_COLOR, 0.6),
		)
	...
	if ui_state.follow_lyrics &&
	   current_lyric_index >= 0 &&
	   current_lyric_index < len(track.lyrics) {
