...
draw_main_content :: proc(sidebar_width: f32) {
	...
	case .PLAYLIST_DETAIL:
		draw_playlist_view(
			...
			content_w,
			content_h,
			find_playlist_by_name(ui_state.selected_playlist),
		)
	case .NOW_PLAYING:
...
frame :: proc() {
	...
	if draw_icon_button_rect(
		...
		maximize_icon,
		fx.BLANK,
		set_alpha(UI_SECONDARY_COLOR, 0.7),
		false,
		6,
	) {
